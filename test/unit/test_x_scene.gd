...
class TestInit:
	...
	func before_all():
		...
		add_child(n)
		n.add_child(c)
		sw = XSceneManager.get_x_scene(n.get_path(), true)
		gut.p("ran run setup")
...
class TestAdd:
	...
	func test_add_key():
		sw.add_scene(s1)
		assert_true(1 in sw.scenes)
		sw.add_scene(s1, sw.count)
		assert_true(2 in sw.scenes)
		sw.add_scene(s1, "hi")
		assert_true("hi" in sw.scenes)
...
class TestShow:
	...
	func test_show_key():
		...
		assert_true(sw.root.is_a_parent_of(s[2].scene))
...
class TestRemove:
	...
	func test_remove_free_deferred():
		...
		for i in range(1, 4):
			s = sw.scenes[i]
			assert_true(is_instance_valid(s.scene))
			sw.remove_scene(i, sw.FREE, true)
			assert_true(is_instance_valid(s.scene))
			yield(get_tree(), "idle_frame")
			assert_false(is_instance_valid(s.scene))
