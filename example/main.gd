...
func tutorial_basics():
	...
	# print(x._stopped_scenes)
	# -> [1, stopped_s1]
	for s in x._stopped_scenes:
		x.show_scene(s)
	# print(x.get_scenes()["stopped_s1"])
	...
	$"World".remove_child(x.scenes[2].scene)
	# print(x._stoppedscenes)
	# -> [2]
	# frees 1 externally
	x.scenes[1].scene.free()
	# print(x.scenes)
	# -> {2:{scene:[Node2D:1247], status:1}}
